...
import (
	...
	"github.com/Univ-Wyo-Education/Blockchain-4010-Fall-2018/Assignments/A-04/config"
...
func (cc *CLI) CreateGenesis(args []string) {
	...
	if lib.Exists(fnIndexPath) {
		fmt.Fprintf(os.Stderr, "Error: %s already exists - you will need to remove it if you want to re-create a new chain.\n", fnIndexPath)
		os.Exit(1)
		return
	}
	...
	for _, act := range cc.GCfg.InitialAccounts {
		out := transactions.TxOutputType{
			Account:     act.Acct,
			Amount:      act.Value,
			TxOutputPos: 0, // Position of the output in this block. In the  block[this].Tx[TxOffset].Output[TxOutptuPos]
